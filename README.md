...
# SuffixSpec / sampled spec default on (`=0` to disable). Verify head defaults to
# the exact `lm_head` (lossless); `MOMIJ_EXACT_HEAD=0` opts into FlashHead probes
# (approximate, faster head — drifts off exact greedy on agentic prompts).
